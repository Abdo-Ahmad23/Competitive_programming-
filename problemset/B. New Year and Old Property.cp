...
void solve(){
    int l,r;cin>>l>>r;
    string s=to_binary(l),k=to_binary(r);
    int cnt=0;
    if(s.size()==k.size() or k.size()-s.size()==1){
        string tmp=s;
        tmp="";
        for(int i=0;i<s.size();i++)tmp+='1';
        for(int i=1;i<s.size();i++){
            tmp[i]='0';
            bitset<64>a(tmp);
            int _1=a.to_ullong();
            if(_1>=l and _1<=r)cnt++;
            tmp[i]='1';
        }
        tmp="";
        for(int i=0;i<k.size();i++)tmp+='1';
    ...
    if(cnt==0)cout<<0<<endl;
        ...
        if(l==r)cout<<cnt-1<<endl;
        else cout<<cnt<<endl;
    }
//    if(l!=r and !(s.size()==k.size() or k.size()-s.size()==1))
//    cout<<cnt<<endl;
